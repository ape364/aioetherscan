...
async def test_total_supply(token):
    with patch('aioetherscan.network.Network.get', new=AsyncMock()) as mock:
        ...
        mock.assert_called_once_with(params=dict(module='stats', action='tokensupply', contractaddress='addr'))
...
async def test_account_balance(token):
    with patch('aioetherscan.network.Network.get', new=AsyncMock()) as mock:
        ...
        mock.assert_called_once_with(
            params=dict(module='account', action='tokenbalance', address='a1', contractaddress='c1', tag='latest')
        )

    with patch('aioetherscan.network.Network.get', new=AsyncMock()) as mock:
        await token.account_balance('a1', 'c1', 123)
        mock.assert_called_once_with(
            params=dict(module='account', action='tokenbalance', address='a1', contractaddress='c1', tag='0x7b')
        )
...
async def test_total_supply_by_blockno(token):
    with patch('aioetherscan.network.Network.get', new=AsyncMock()) as mock:
        ...
        mock.assert_called_once_with(
            params=dict(module='stats', action='tokensupplyhistory', contractaddress='c1', blockno=123)
        )
...
async def test_account_balance_by_blockno(token):
    with patch('aioetherscan.network.Network.get', new=AsyncMock()) as mock:
        ...
        mock.assert_called_once_with(
            params=dict(module='account', action='tokenbalancehistory', address='a1', contractaddress='c1', blockno=123)
        )
...
async def test_token_holder_list(token):
    with patch('aioetherscan.network.Network.get', new=AsyncMock()) as mock:
        ...
        mock.assert_called_once_with(
            params=dict(module='token', action='tokenholderlist', contractaddress='c1', page=None, offset=None)
        )

    with patch('aioetherscan.network.Network.get', new=AsyncMock()) as mock:
        await token.token_holder_list('c1', 1, 10)
        mock.assert_called_once_with(
            params=dict(module='token', action='tokenholderlist', contractaddress='c1', page=1, offset=10)
        )
...
async def test_token_holding_erc20(token):
    with patch('aioetherscan.network.Network.get', new=AsyncMock()) as mock:
        ...
        mock.assert_called_once_with(
            params=dict(module='account', action='addresstokenbalance', address='a1', page=None, offset=None)
        )

    with patch('aioetherscan.network.Network.get', new=AsyncMock()) as mock:
        await token.token_holding_erc20('a1', 1, 10)
        mock.assert_called_once_with(
            params=dict(module='account', action='addresstokenbalance', address='a1', page=1, offset=10)
        )
...
async def test_token_holding_erc721(token):
    with patch('aioetherscan.network.Network.get', new=AsyncMock()) as mock:
        ...
        mock.assert_called_once_with(
            params=dict(module='account', action='addresstokennftbalance', address='a1', page=None, offset=None)
        )

    with patch('aioetherscan.network.Network.get', new=AsyncMock()) as mock:
        await token.token_holding_erc721('a1', 1, 10)
        mock.assert_called_once_with(
            params=dict(module='account', action='addresstokennftbalance', address='a1', page=1, offset=10)
        )
