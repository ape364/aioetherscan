...
async def test_is_contract_ok(contract_utils):
    ...
    with patch('aioetherscan.modules.contract.Contract.contract_abi', new=AsyncMock(return_value=return_value)) as mock:
...
async def test_is_contract_negative(contract_utils):
    ...
    with patch('aioetherscan.modules.contract.Contract.contract_abi', new=AsyncMock(side_effect=exc)) as mock:
        result = await contract_utils.is_contract(address='addr')
...
async def test_get_contract_creator_internal_ok(contract_utils):
    creator = '0x123'
    internal_txs_mock = AsyncMock(return_value=[{'from': creator}, ])
...
async def test_get_contract_creator_internal_raise_none(contract_utils):
    ...
    creator = '0x123'
    normal_txs_mock = AsyncMock(return_value=[{'from': creator}, ])
    with patch('aioetherscan.modules.account.Account.internal_txs', new=internal_txs_mock) as internal_mock:
        with patch('aioetherscan.modules.account.Account.normal_txs', new=normal_txs_mock) as normal_mock:
            result = await contract_utils.get_contract_creator(contract_address='addr')
            internal_mock.assert_called_once_with(
                address='addr',
                start_block=1,
                page=1,
                offset=1
